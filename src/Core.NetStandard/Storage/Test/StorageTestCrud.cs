...
namespace Xlent.Lever.Libraries2.Core.Storage.Test
{
    ...
    public class StorageTestCrud<TStorage, TTestableItem, TId>
        ...
        where TTestableItem : IStorableItemForTesting<TTestableItem, TId>, IStorableItem<TId>, IValidatable, new()
    {
        /// <summary>
        ...
        public StorageTestCrud(TStorage storage)
        {
            Storage = storage;
        }
        ...
        /// </summary>
        public async Task RunAllSunshineTests()
        {
            await PrepareTestCase();
            await Create();
            await PrepareTestCase();
            await Read();
            await PrepareTestCase();
        ...
        {
            var methodName = $"{typeof(StorageTestCrud<TStorage, TTestableItem, TId>).FullName}.{typeof(StorageTestCrud<TStorage, TTestableItem, TId>).GetTypeInfo().GetDeclaredMethod(nameof(Create)).Name}";
        ...
        {
            var methodName = $"{typeof(StorageTestCrud<TStorage, TTestableItem, TId>).FullName}.{typeof(StorageTestCrud<TStorage, TTestableItem, TId>).GetTypeInfo().GetDeclaredMethod(nameof(Read)).Name}";
            var initialItem = new TTestableItem().InitializeWithDataForTesting(TypeOfTestDataEnum.Variant1);
